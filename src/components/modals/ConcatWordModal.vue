...
<template>
	<NModal :segmented="{ footer: 'soft' }" :show="dialogs.concatWords" :title="t('concatWordModal.title')"
		preset="card" style="max-width: 600px;" transform-origin="center" @close="dialogs.concatWords = false">
		...
		<NFormItem :label="t('concatWordModal.mergeRange')">
			<NSlider v-model:value="mergeRange" :format-tooltip="(v: number) => `${v + 1}`"
				:max="originPreview.length - 1" :min="0" :step="1" range />
		</NFormItem>
		...
		<NSpace vertical>
			...
			<div class="word-preview">
				<span v-for="(word, i) in concatPreview" :class="{ 'merge-word': i === mergeRangeMin }">
					{{ word }}
				</span>
			</div>
		</NSpace>
		<template #footer>
			<NButton type="primary" @click="processWordConcat">
...
const emptyBeatMergeOptions: SelectOption[] = [
	{
		label: t("concatWordModal.emptyBeatMergeMethod.accumulateWordBeat"),
		value: "accumulate-word-beat",
	},
	{
		label: t("concatWordModal.emptyBeatMergeMethod.accumulateBeat"),
		value: "accumulate-beat",
	},
	{
		label: t("concatWordModal.emptyBeatMergeMethod.accumulateWord"),
		value: "accumulate-word",
	},
...
const originPreview = computed(
	() =>
		lyric.lyrics[rightClick.selectedLine]?.words?.map((word) => word.word) ??
		[],
);
const concatPreview = computed(() => {
	const origin = [...originPreview.value];
	const joined = origin
		.slice(mergeRangeMin.value, mergeRangeMax.value + 1)
		.join("");
	origin.splice(
		mergeRangeMin.value,
		mergeRangeMax.value - mergeRangeMin.value + 1,
		joined,
	);
	return origin;
});
		...
		if (concatWords.length > 1) {
			...
			switch (emptyBeatMergeMethod.value) {
				...
				case "accumulate-beat":
					newWord.emptyBeat = concatWords.reduce(
						(acc, cur) => acc + (cur.emptyBeat ?? 0),
						0,
					);
					break;
				case "accumulate-word-beat":
					newWord.emptyBeat =
						concatWords.length -
						1 +
						concatWords.reduce((acc, cur) => acc + (cur.emptyBeat ?? 0), 0);
					break;
				default:
			}
			const words = (window.structuredClone || structuredClone)(
				toRaw(line.words),
			);
